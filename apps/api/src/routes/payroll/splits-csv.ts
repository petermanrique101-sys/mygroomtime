...
import { renderPayrollCsv, payrollCsvFilename } from '../../services/payroll-csv.js';

export default async function payrollSplitsCsvRoute(app: FastifyInstance): Promise<void> {
  app.get(
    '/payroll/splits.csv',
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const tenant = await db.global.tenant.findUnique({
        ...
        select: { payrollPeriodKind: true, slug: true },
      });
      if (!tenant) {
      ...
      const csv = renderPayrollCsv(result);
      const filename = payrollCsvFilename(tenant.slug, result.period.periodStart);
      reply
